#!/usr/bin/env node
/**
 * Retro Arcade Space Defender Jet & Heatmap Generator for @codewitdean
 * 
...
export const USERNAME = process.env.GH_USERNAME || "codewitdean";
...
export function selectTargets(cells) {
  ...
  return selectedCols.map(col => {
    ...
    const { t_fwd, t_ret } = columnToTimeFraction(col, COLS - 1);

    const k_lock_fwd = Math.max(0.0001, t_fwd - 0.008);
    const k_hit_fwd = t_fwd;
    const k_settle_fwd = Math.min(0.4998, t_fwd + 0.008);

    const k_lock_ret = Math.max(0.5001, t_ret - 0.008);
    const k_hit_ret = t_ret;
    const k_settle_ret = Math.min(0.9998, t_ret + 0.008);
...
${buildLegend()}
<text x="${WIDTH - 140}" y="${HEIGHT - 12}" class="hud-meta" text-anchor="end">[ARCADE DEFENSE GRID // SECTOR: CODEWITDEAN]</text>
...
export async function main() {
  console.log("Initializing Retro Arcade Space Defender Heatmap Engine for @codewitdean...");
  ...
  // Also sync to root github-jet.svg
